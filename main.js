...
setInterval(() => {
  ...
  const scashare=localStorage.getItem("uplcc_sca_share_code");
  if(scashare){
    const share=JSON.parse(scashare);
    const times=Number(share.times||0);
    const code=share.code;
    if(times==0&&code){
      const whether=window.confirm("检测到来自sca的在线预览请求，是否确认替换代码");
      if(whether){
        document.getElementById("tex").value=code;
        glh();
      }
      localStorage.setItem("uplcc_sca_share_code",JSON.stringify({times:1,code}));
    }
  }
}, 1000);
function runCode() {
...
function glh() {
  ...
  const esca = glll.map(k => k.replace(/[.*+?^${}()|]/g, '\\$&'));
  const escb = gllla.map(k => k.replace(/[.*+?^${}()|]/g, '\\$&'));
  const escc = glllb.map(k => k.replace(/[.*+?^${}()|]/g, '\\$&'));
  ...
  glhtml = glhtml
             .replace(/\"(.*?)\"/g, `<span class='glr'>\"$1\"</span>`)
             .replace(/`(.*?)`/g, `<span class='glr'>\`$1\`</span>`);
  gl.innerHTML = glhtml;
}
glh();
